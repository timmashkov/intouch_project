...
import aio_pika
from aio_pika.message import Message, AbstractIncomingMessage, IncomingMessage
...
class RPC(BaseMQ):
    ...
    async def cancel_consumer(queue, consumers):
        async for key, val in consumers.items():
...
mq_rpc = RPC(main_config.rabbit_url)
mq_handler = MessageQueue(main_config.rabbit_url)
